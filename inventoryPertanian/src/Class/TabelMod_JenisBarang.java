 ...
 * @author user
 */
public abstract class TabelMod_JenisBarang extends AbstractTableModel{
    
    private List<Model_JenisBarang> list = new ArrayList<>();
    
    public void tambahData(Model_JenisBarang mod_jb){
        list.add(mod_jb);
        fireTableRowsInserted(list.size() - 1, list.size() -1);
        JOptionPane.showMessageDialog(null, "Data berhasil ditambahkan");
    }
    
    public void perbaruiData(int row, Model_JenisBarang mod_jb){
        list.add(row, mod_jb);
    ...
    public void setData(int index, Model_JenisBarang mod_jb){
        list.set(index, mod_jb);
